...
func assembleInlineQueryResult(u *url.URL, shortener monstrator.Shortener, r *http.Request) *inlineQueryResultArticle {
...
func handleInlineQuery(w http.ResponseWriter, r *http.Request, q *inlineQuery) {
	...
	if ok, shortener := isShortenedURL(u); ok {
		...
		results = []interface{}{assembleInlineQueryResult(longURL, shortener, r)}
	} else {
		wg := sync.WaitGroup{}
		...
		shorten := func(shortener monstrator.Shortener) {
			...
			if err != nil {
				...
				c <- assembleInlineQueryResult(shortenedURL, shortener, r)
			}
		}
	...
	default:
		...
		case "start":
			if m.Sender == nil || m.Sender.FirstName == "" {
				text = "Hello!\nI can shorten and expand URLs through [inline queries](https://core.telegram.org/bots/inline)."
			} else {
				text = fmt.Sprintf("Hello %s!\nI can shorten and expand URLs through [inline queries](https://core.telegram.org/bots/inline).", m.Sender.FirstName)
			}
